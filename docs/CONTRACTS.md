...
Review follow-up changes (same T0 baseline, before any consumer task started):

7. **`CompletionOutcome` gains `cancelled`**: the finish report may carry `cancelled` when the worker stopped because of a `stop` request. The reaper's expired-lease reconciliation (`lifecycle.expired_lease_run_outcome`) now maps the persisted manifest outcome as-is — `succeeded`/`no_changes` → `SUCCEEDED`, `tests_failed`/`failed`/`interrupted` → `FAILED` preserving the manifest's original error code, `cancelled` → `CANCELLED`, no manifest → `FAILED(lost_lease)`. A persisted `tests_failed`/`cancelled`/`interrupted` can never be reconciled into `SUCCEEDED`.
8. **Python projections pinned to the published closed schemas** (machine contract unchanged): `agy_cloud.models.Agent` now carries the nested `snapshot` object (`gcs_uri`, `seq`, `conversation_id`, `agy_version`, `updated_at`) and typed `stats` exactly as published (previously flattened/loose); `OutboundWebhookEvent.usage` is removed from the Python model — the published closed schema never declared it. Parity tests machine-check `Agent`, `Run`, and `OutboundWebhookEvent` field-by-field (including nested objects and enums) against `openapi.yaml`, so future drift fails `make test`.
9. **Internal identity error-code scope made explicit**: `forbidden_worker` = worker identity presented on scheduler-only `/internal/tick` (machine example on the route); `forbidden_scheduler` = scheduler identity presented on a worker route (documented on the `workerIdToken` security scheme).
10. **`pop` lease-held precondition stated in both contracts**: only a currently held lease with matching `(session_id, generation)` may pop; released/rebound → `409 lease_not_held` (an old session never steals an unbound run), stale/never-issued generation → `409 generation_mismatch`, no mutation on rejection (`lifecycle.check_pop_authorization`).
...
- `pop` additionally requires the presented session to currently hold the agent lease: a released lease (runs left unbound) or one rebound to a different session is `409 lease_not_held` — an old session must never steal an unbound run; unbound runs wait for the scheduler to bind them to a new session. A stale or never-issued generation is `409 generation_mismatch`. Rejected pops mutate nothing (`lifecycle.check_pop_authorization`).
...
- Expired-lease recovery (reaper): the reaper first reads the persisted completion manifest and reconciles its outcome as-is (`lifecycle.expired_lease_run_outcome`) — `succeeded`/`no_changes` stay `SUCCEEDED`; `tests_failed`/`failed`/`interrupted` stay `FAILED` preserving the manifest's original error code; `cancelled` stays `CANCELLED`. Without a manifest the run is `FAILED(lost_lease)`, the slot/VM/disk are released, and nothing is replayed. A persisted failing/cancelled outcome is never overwritten as successful.
...
- `tests_failed`: model SUCCESS with failing tests → run `FAILED(tests_failed)`, Check Run failure, optional draft PR; never shown as a successful task. Machine-enforced by `validate_completion_manifest`: outcome `tests_failed` requires `tests.status=failed` and `error_code=tests_failed`; outcome `succeeded` cannot carry `tests.status=failed`; `failed`/`interrupted` require `error_code`.
...
1. **Scheduler** — `POST /internal/tick` accepts ONLY a Google-signed ID token whose subject is the scheduler service account (`agy-sched@<project>`), with audience = the service URL. A worker identity on tick is `403 forbidden_worker`; a session capability NEVER authorizes tick; API keys cannot reach `/internal` at all. A scheduler identity presented on a worker route is `403 forbidden_scheduler` — the realms never cross in either direction.
